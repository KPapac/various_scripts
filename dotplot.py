...
=======
        chunk_size = 10**6  # Define a reasonable chunk size
>>>>>>> baf4c3a7c63a7cb09d60f762e79e5a7d1f387c63
...
=======
        print("Hint: Use a tool like `head` or `pandas.read_csv` with a small sample to inspect the file and check available columns.")
>>>>>>> baf4c3a7c63a7cb09d60f762e79e5a7d1f387c63
        return
...
=======
            data[x_column]
            .apply(lambda x: pd.NA if x <= 0 else x)
            .dropna()
    ...
    if log_y:
        data[y_column] = (
            data[y_column]
            .apply(lambda y: pd.NA if y <= 0 else y)
            .dropna()
            .apply(lambda y: np.log(y))
        )
