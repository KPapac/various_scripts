#!/usr/bin/env python3
#This script generates some basic genome stats from a embl flat file format (.embl)
#Run like: gb_stats.py file.embl
...
parser = argparse.ArgumentParser(
    description='''Script that takes embl file input and computes statistics ''',
    epilog="""Jon Palmer (2015)  palmer.jona@gmail.com""",
)
parser.add_argument('embl', help='embl file (Required)')
args = parser.parse_args()
if len(sys.argv) < 2:
    parser.print_usage()
    sys.exit(1)
embl_filename = args.embl
gene_length = 0
avg_gene_length = 0
num_records = list(SeqIO.parse(embl_filename, "embl"))
...
G_count = 0
seqlength = []
for record in SeqIO.parse(embl_filename, "embl"):
...
print("Total bp:\t{:,} {}".format(total, ending))
